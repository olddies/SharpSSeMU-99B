...
static_assert(std::size(k099BBaseClasses) == std::size(k099BEvolvedClasses),
              "cada clase base necesita su entrada de evolucion, aunque sea vacia");

}  // namespace

CLASS_TYPE CCharacterManager::ChangeServer099BClassTypeToClientClassType(const BYTE byBaseClass,
                                                                        const BYTE byChangeUp)
{
    if (byBaseClass >= std::size(k099BBaseClasses))
    {
    ...
    if (byChangeUp == 0)
    {
        return k099BBaseClasses[byBaseClass];
    }

    return k099BEvolvedClasses[byBaseClass];
}

BYTE CCharacterManager::ChangeClientClassTypeToServer099BBaseClass(const CLASS_TYPE byClientClassType)
{
