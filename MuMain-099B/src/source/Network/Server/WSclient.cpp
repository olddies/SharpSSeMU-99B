...
#include "Protocol099B/Send099B.h"
...
void ReceiveServerList(const BYTE* ReceiveBuffer)
{
    ...
    g_ServerListManager->SetTotalServer(Data->count);

    for (int i = 0; i < g_ServerListManager->GetTotalServer(); i++)
    {
        auto Row = reinterpret_cast<const Mu099B::PMSG_SERVER_LIST*>(ReceiveBuffer + Offset);

        g_ServerListManager->InsertServerGroup(Row->ServerCode, Row->UserTotal);

        g_ConsoleDebug->Write(MCD_RECEIVE, L"0xF4:02 [ReceiveServerList(%d code=%d carga=%d%%)]",
            i, Row->ServerCode, Row->UserTotal);

        Offset += sizeof(Mu099B::PMSG_SERVER_LIST);
    }
...
void ReceiveServerConnect(const BYTE* ReceiveBuffer)
{
    ...
    memcpy(Address, Data->ServerAddress, sizeof(Data->ServerAddress));

    wchar_t IP[sizeof(Address)];
    CMultiLanguage::ConvertFromUtf8(IP, Address);
    ...
    if (CreateSocket(IP, Data->ServerPort))
    {
        g_bGameServerConnected = TRUE;
    }

    wchar_t Text[100];
    mu_swprintf(Text, I18N::Game::YouAreConnectedToTheServer, IP, Data->ServerPort);
...
void ReceiveServerConnectBusy(const BYTE* ReceiveBuffer)
{
    ...
    Mu099B::SendServerListRequest(*SocketClient);
}
...
void ReceiveCharacterList099B(const BYTE* ReceiveBuffer)
{
    InitGuildWar();
    ...
    g_ConsoleDebug->Write(MCD_RECEIVE, L"0xF3:00 [ReceiveCharacterList(%d personaje(s))]",
        envelope->count);
    ...
    for (int i = 0; i < envelope->count; i++)
    {
        ...
        switch (Row->slot)
        {
            ...
            default: continue;  // un slot fuera de rango no puede tirar el resto de la lista
        }

        const auto iClass = gCharacterManager.ChangeServer099BClassTypeToClientClassType(
            appearance.CharacterClass, appearance.ChangeUp);

        CHARACTER* c = CreateHero(Row->slot, iClass, 0, fPos[0], fPos[1], fAngle);

        c->Level = Row->Level;
        c->CtlCode = Row->CtlCode;

        memset(c->ID, 0, sizeof(c->ID));
        CMultiLanguage::ConvertFromUtf8(c->ID, Row->Name, MAX_USERNAME_SIZE);

        BYTE equipment[Mu099B::ExtendedEquipmentSize];
...
void ReceiveCreateCharacter(const BYTE* ReceiveBuffer)
{
    ...
    if (Data->result == 1)
    {
        ...
        CharactersClient[Data->slot].Class = iClass;
        CharactersClient[Data->slot].SkinIndex = gCharacterManager.GetSkinModelIndex(iClass);
        CMultiLanguage::ConvertFromUtf8(CharactersClient[Data->slot].ID, Data->name,
            MAX_USERNAME_SIZE);
        CharactersClient[Data->slot].ID[MAX_USERNAME_SIZE] = L'\0';
    ...
    g_ConsoleDebug->Write(MCD_RECEIVE, L"0xF3:01 [ReceiveCreateCharacter(result=%d)]", Data->result);
}
...
void InitGame()
{
    ...
    Mu099B::SendNpcClose(*SocketClient);
...
BOOL ReceiveLogOut(const BYTE* ReceiveBuffer, BOOL bEncrypted)
{
    ...
    case 1:
        ...
        Mu099B::SendCharacterListRequest(*SocketClient);
...
BOOL ReceiveJoinMapServer(std::span<const BYTE> ReceiveBuffer)
{
    ...
    const auto* Data =
        reinterpret_cast<const Mu099B::PMSG_CHARACTER_INFO_SEND*>(ReceiveBuffer.data());

    CharacterAttribute->Experience = Data->Experience;
    CharacterAttribute->NextExperience = Data->NextExperience;
    CharacterAttribute->LevelUpPoint = Data->LevelUpPoint;

    CharacterAttribute->Strength = Data->ViewStrength;
    CharacterAttribute->Dexterity = Data->ViewDexterity;
    CharacterAttribute->Vitality = Data->ViewVitality;
    CharacterAttribute->Energy = Data->ViewEnergy;
    CharacterAttribute->Charisma = Data->ViewLeadership;

    CharacterAttribute->Life = static_cast<float>(Data->ViewCurHP);
    CharacterAttribute->LifeMax = static_cast<float>(Data->ViewMaxHP);
    CharacterAttribute->Mana = static_cast<float>(Data->ViewCurMP);
    CharacterAttribute->ManaMax = static_cast<float>(Data->ViewMaxMP);
    ...
    CharacterMachine->Gold = Data->Money;
    ...
    CreateCharacterPointer(c, MODEL_PLAYER, Data->X, Data->Y,
        (static_cast<float>(Data->Dir) - 1.f) * 45.f);
    c->Key = HeroKey;

    g_ConsoleDebug->Write(MCD_RECEIVE, L"0xF3:03 [ReceiveJoinMapServer] Key: %d Map: %d X: %d Y: %d",
        c->Key, gMapManager.WorldActive, Data->X, Data->Y);
...
BOOL ReceiveInventory099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    for (int i = 0; i < Data->Value; i++)
    {
        ...
        const auto item = Mu099B::DecodeItemInfo(&ReceiveBuffer[Offset + 1]);
        Offset += RowSize;
        ...
        const std::span<const BYTE> itemData(block, length);
    ...
    g_ConsoleDebug->Write(MCD_RECEIVE, L"0xF3:10 [ReceiveInventory(%d item(s))]", Data->Value);
...
void ReceiveShopItemList099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    const bool isMix = (Data->SubCode == ChaosBoxList);
    ...
    for (int i = 0; i < Data->Value; i++)
    {
        if (Offset + RowSize > ReceiveBuffer.size())
        {
            g_ErrorReport.Write(L"[ShopItemList] paquete corto: %d entradas declaradas, "
                L"cortado en la %d", Data->Value, i);
            return;
        }
...
void ReceiveBuy099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    {
        g_pNewUISystem->HideAll();
        g_pChatListBox->AddText(Hero->ID, I18N::Game::CannotBeTraded,
            SEASON3B::TYPE_ERROR_MESSAGE);
        BuyCost = 0;
        return;
    }

    if (Data->result == BuyFailedSilent)
    {
        BuyCost = 0;
        return;
    }
    ...
    if (IsMainInventorySlot(Data->result))
    {
        g_pMyInventory->InsertItem(Data->result, itemData);
    }
    else if (IsInventoryExtensionSlot(Data->result))
    {
        g_pMyInventoryExt->InsertItem(Data->result, itemData);
    }
...
void ReceiveChat(const BYTE* ReceiveBuffer)
{
    ...
    else
    {
        ...
        CMultiLanguage::ConvertFromUtf8(ID, Data->name, MAX_USERNAME_SIZE);
        ...
        CMultiLanguage::ConvertFromUtf8(Text, Data->message, MessageSize);
...
void ReceiveChatWhisper(const BYTE* ReceiveBuffer)
{
    ...
    auto Data = reinterpret_cast<const Mu099B::PMSG_CHAT_WHISPER_SEND*>(ReceiveBuffer);
...
void ReceiveMoveCharacter099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    const auto* Data = reinterpret_cast<const Mu099B::PMSG_MOVE_SEND*>(ReceiveBuffer.data());
    ...
    c->TargetX = Data->x;
    c->TargetY = Data->y;
    c->TargetAngle = Data->dir;

    if (c->Appear == 0 && PathFinding2(c->PositionX, c->PositionY, c->TargetX, c->TargetY, &c->Path))
    {
        c->Movement = true;
    }
    else if (!c->Movement)
    ...
    g_ConsoleDebug->Write(MCD_RECEIVE, L"0xD7 [Move] %ls key=%d -> (%d,%d) dir=%d",
        c->ID, Key, Data->x, Data->y, Data->dir);
}
...
{
    auto Data = safe_cast<Mu099B::PMSG_USER_DIE_SEND>(ReceiveBuffer);
    if (Data == nullptr)
    {
        assert(false);
        return;
    ...
    PHEADER_DEFAULT_DIE adapted{};
    adapted.KeyH = Data->index[0];
    adapted.KeyL = Data->index[1];

    // 0.99B manda el skill en un solo byte; el otro lado espera dos.
    adapted.MagicH = 0;
    adapted.MagicL = Data->skill;

    adapted.TKeyH = Data->killer[0];
    adapted.TKeyL = Data->killer[1];

    ReceiveDie(reinterpret_cast<const BYTE*>(&adapted), sizeof(adapted));
}

BOOL ReceiveTeleport099B(std::span<const BYTE> ReceiveBuffer, BOOL bEncrypted)
{
    auto Data = safe_cast<Mu099B::PMSG_TELEPORT_SEND>(ReceiveBuffer);
    if (Data == nullptr)
    {
        assert(false);
        return FALSE;
    ...
    PRECEIVE_TELEPORT_POSITION adapted{};
    adapted.Flag = Data->gate;
    adapted.Map = Data->map;
    adapted.PositionX = Data->x;
    adapted.PositionY = Data->y;
    adapted.Angle = Data->dir;

    return ReceiveTeleport(reinterpret_cast<const BYTE*>(&adapted), bEncrypted);
}
...
void ReceiveEquipment099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    const auto appearance = Mu099B::DecodeCharSet(Data->CharSet);
    ...
    c->Class = gCharacterManager.ChangeServer099BClassTypeToClientClassType(
        appearance.CharacterClass, appearance.ChangeUp);
    c->SkinIndex = gCharacterManager.GetSkinModelIndex(c->Class);

    g_ConsoleDebug->Write(MCD_RECEIVE, L"0xF3:13 [ReceiveEquipment(key 0x%04X)]", Key);
}
...
void ReceiveCreatePlayerViewport099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    if (ReceiveBuffer.size() < sizeof(Mu099B::PMSG_VIEWPORT_SEND))
    {
        return;
    }

    const auto* envelope =
        reinterpret_cast<const Mu099B::PMSG_VIEWPORT_SEND*>(ReceiveBuffer.data());
    size_t offset = sizeof(Mu099B::PMSG_VIEWPORT_SEND);

    for (int i = 0; i < envelope->count; ++i)
    {
        if (offset + EntrySize > ReceiveBuffer.size())
        {
            g_ErrorReport.Write(L"[ViewportPlayer] paquete corto: %d entradas declaradas, "
                L"cortado en la %d", envelope->count, i);
            break;
        }
        ...
        memset(c->ID, 0, sizeof c->ID);
        CMultiLanguage::ConvertFromUtf8(c->ID, Entry->name, MAX_USERNAME_SIZE);
        ...
        c->PK = static_cast<BYTE>(Entry->DirAndPkLevel & 0x0F);
        o->Angle[2] = (static_cast<float>(Entry->DirAndPkLevel >> 4) - 1.f) * 45.f;
        ...
        case 1:
            ...
            break;
        case 2:
            SetAction(o, gCharacterManager.IsFemale(c->Class) ? PLAYER_SIT_FEMALE1 : PLAYER_SIT1);
            break;
        case 3:
            SetAction(o, gCharacterManager.IsFemale(c->Class) ? PLAYER_POSE_FEMALE1 : PLAYER_POSE1);
            break;
        case 4:
            SetAction(o,
                gCharacterManager.IsFemale(c->Class) ? PLAYER_HEALING_FEMALE1 : PLAYER_HEALING1);
            break;
        default:
            break;
        }
        ...
        if (CreateFlag)
        {
            o->Position[0] = (c->PositionX + 0.5f) * TERRAIN_SCALE;
            o->Position[1] = (c->PositionY + 0.5f) * TERRAIN_SCALE;
            CreateEffect(BITMAP_MAGIC + 2, o->Position, o->Angle, o->Light, 0, o);
            o->Alpha = 0.f;
        }
        else if (PathFinding2(c->PositionX, c->PositionY, Entry->tx, Entry->ty, &c->Path))
        {
            c->Movement = true;
        }
        ...
        BYTE equipment[Mu099B::ExtendedEquipmentSize];
        Mu099B::WriteExtendedEquipment(appearance, equipment);
        ReadEquipmentExtended(FindCharacterIndex(Key), 0, equipment);

        g_ConsoleDebug->Write(MCD_RECEIVE,
            L"0x12 [ViewportPlayer] %ls key=%d (%d,%d) -> (%d,%d)",
            c->ID, Key, c->PositionX, c->PositionY, c->TargetX, c->TargetY);
    }
}
...
void ReceiveCreateMonsterViewport099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    for (int i = 0; i < envelope->count; ++i)
    {
        if (offset + EntrySize > ReceiveBuffer.size())
        {
            g_ErrorReport.Write(L"[ViewportMonster] paquete corto: %d entradas declaradas, "
                L"cortado en la %d", envelope->count, i);
            break;
        }
        ...
        const auto Type =
            static_cast<EMonsterType>((static_cast<WORD>(Entry->type[0]) << 8) | Entry->type[1]);

        CHARACTER* c = CreateMonster(Type, Entry->x, Entry->y, Key);
        if (c == nullptr)
        {
            continue;  // un tipo desconocido no puede tirar el resto de la lista
        }
        ...
        g_ConsoleDebug->Write(MCD_RECEIVE, L"0x13 [ViewportMonster] tipo=%d key=%d (%d,%d)",
            static_cast<int>(Type), Key, Entry->x, Entry->y);
    }
}
...
void ReceiveMagicList099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    constexpr BYTE RemoveOne = 0xFF;
    constexpr BYTE AddOne = 0xFE;
    constexpr size_t RowSize = 3;

    size_t Offset = sizeof(Mu099B::PMSG_SKILL_LIST_SEND);

    auto readRow = [&](size_t at, BYTE& slot, WORD& skill, BYTE& level) {
        slot = ReceiveBuffer[at];
        const BYTE low = ReceiveBuffer[at + 1];
        const BYTE high = ReceiveBuffer[at + 2];
        skill = static_cast<WORD>(low + (high & 0x07) * 255);
        level = static_cast<BYTE>(high >> 3);
    };

    const bool single = (Data->count == RemoveOne || Data->count == AddOne);
    const int rows = single ? 1 : Data->count;

    for (int i = 0; i < rows; ++i)
    {
        if (Offset + RowSize > ReceiveBuffer.size())
        {
            g_ErrorReport.Write(L"[SkillList] paquete corto: %d entradas declaradas, "
                L"cortado en la %d", Data->count, i);
            return;
        }
        ...
        WORD skill = 0;
        BYTE level = 0;
        readRow(Offset, slot, skill, level);
        Offset += RowSize;

        if (slot >= MAX_SKILLS)
        {
            continue;  // un slot fuera de rango no puede pisar memoria ajena
        }

        if (Data->count == RemoveOne)
        {
            CharacterAttribute->Skill[slot] = AT_SKILL_UNDEFINED;
            CharacterAttribute->SkillLevel[slot] = 0;
        }
        else
        {
            CharacterAttribute->Skill[slot] = static_cast<ActionSkillType>(skill);
            CharacterAttribute->SkillLevel[slot] = level;
        }
    }
    ...
    for (auto& s : CharacterAttribute->Skill)
    {
        if (s != AT_SKILL_UNDEFINED)
        {
            CharacterAttribute->SkillNumber++;
            if (SkillAttribute[s].SkillUseType == SKILL_USE_TYPE_MASTER)
            {
                CharacterAttribute->SkillMasterNumber++;
            }
        }
    ...
    g_ConsoleDebug->Write(MCD_RECEIVE, L"0xF3:11 [SkillList(%d)]", Data->count);
}
...
void ReceiveAttackDamage099B(const BYTE* ReceiveBuffer)
{
    ...
    const bool Missed = (Data->index[0] & 0x80) != 0;
    const int Key = ((Data->index[0] & 0x7F) << 8) | Data->index[1];

    const int Index = FindCharacterIndex(Key);
    CHARACTER* c = &CharactersClient[Index];
    OBJECT* o = &c->Object;

    const int Damage = static_cast<int>(Data->ViewDamageHP);
    const int DamageType = Data->type & 0x0F;
...
void ReceiveExperience099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    const int Key = (static_cast<int>(Data->index[0]) << 8) | Data->index[1];
    const DWORD damage = Data->ViewDamageHP;

    const uint64_t previous = CharacterAttribute->Experience;

    CharacterAttribute->Experience = Data->ViewExperience;
    CharacterAttribute->NextExperience = Data->ViewNextExperience;

    const uint64_t gained = CharacterAttribute->Experience > previous
    ...
    if (Index >= 0)
    {
        ...
        vec3_t Light;
        Vector(1.f, 0.6f, 0.f, Light);
        CreatePoint(c->Object.Position, static_cast<int>(damage), Light);
    }

    if (gained > 0)
    {
        wchar_t Text[100];
        mu_swprintf(Text, I18N::Game::ObtainedDExp, static_cast<DWORD>(gained));
        g_pSystemLogBox->AddText(Text, SEASON3B::TYPE_SYSTEM_MESSAGE);
    }

    g_ConsoleDebug->Write(MCD_RECEIVE, L"0x9C [Experiencia +%d, dano %d]",
        static_cast<DWORD>(gained), damage);
}
...
void ReceiveCreateItemViewport099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    for (int i = 0; i < Data->Value; i++)
    {
        if (Offset + RowSize > ReceiveBuffer.size())
        {
            g_ErrorReport.Write(L"[ViewportItem] paquete corto: %d entradas declaradas, "
                L"cortado en la %d", Data->Value, i);
            return;
        }
        ...
        const BYTE x = ReceiveBuffer[Offset + 2];
        const BYTE y = ReceiveBuffer[Offset + 3];

        const BYTE* itemBytes = &ReceiveBuffer[Offset + 4];
        const auto item = Mu099B::DecodeItemInfo(itemBytes);
        Offset += RowSize;

        if (id < 0 || id >= MAX_ITEMS)
        {
            continue;  // sin lugar donde guardarlo
        }
        ...
        {
            CreateMoneyDrop(&Items[id], money, Position, isFreshDrop);
            MUHelper::g_MuHelper.AddItem(id, {x, y});
            continue;
        }
        ...
        const auto params = ParseItemData(std::span<const BYTE>(block, length));
...
void ReceiveGetItem099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    if (Data->result == GET_ITEM_ZEN)
    {
        ...
        CharacterMachine->Gold =
            ...
            (Data->ItemInfo[2] << 8) | Data->ItemInfo[3];

        const int getGold = CharacterMachine->Gold - backupGold;
        if (getGold > 0)
        {
            wchar_t szMessage[128];
    ...
    const std::span<const BYTE> itemData(block, length);
    const int itemIndex = Data->result;
    ...
    {
        SendGetItem = -1;
        return;  // nada que nombrar en el log
    }

    wchar_t szItem[64] = {0};
    GetItemName(pickedItem->Type, pickedItem->Level, szItem);
...
BOOL ReceiveEquipmentItem099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    auto Data = safe_cast<Mu099B::PMSG_ITEM_MOVE_SEND>(ReceiveBuffer);
    if (Data == nullptr)
    {
        assert(false);
        return false;
    ...
    constexpr BYTE MoveFailed = 0xFF;
    ...
    if (Data->result == MoveFailed)
    {
        ...
        g_ConsoleDebug->Write(MCD_RECEIVE, L"0x24 [ReceiveEquipmentItem(fallo)]");
        return TRUE;
    }
    ...
    else if (itemindex >= 0 && itemindex < MAX_EQUIPMENT_INDEX)
    {
        g_pMyInventory->EquipItem(itemindex, itemData);
    }
    else if (IsMainInventorySlot(itemindex))
    ...
    g_ConsoleDebug->Write(MCD_RECEIVE, L"0x24 [ReceiveEquipmentItem(slot %d, indice %d)]",
...
void ReceiveModifyItem099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    g_ConsoleDebug->Write(MCD_RECEIVE, L"0xF3:14 [ReceiveModifyItem099B(slot %d)]", itemindex);
}
...
{
    auto Data = safe_cast<Mu099B::PMSG_TRADE_ITEM_ADD_SEND>(ReceiveBuffer);
    if (Data == nullptr)
    {
        assert(false);
        return;
    ...
    g_pTrade->ProcessToReceiveYourItemAdd(Data->slot,
        std::span<const BYTE>(block, length));
}
...
void ReceiveLevelUp099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    CharacterAttribute->Level = Data->Level;
    CharacterAttribute->LevelUpPoint = static_cast<WORD>(Data->ViewPoint);
...
constexpr BYTE StatCurrent = 0xFF;
constexpr BYTE StatMaximum = 0xFE;
...
void ReceiveNewCharacterInfo099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    CharacterAttribute->Life = Data->ViewCurHP;
    CharacterAttribute->LifeMax = Data->ViewMaxHP;
    CharacterAttribute->Mana = Data->ViewCurMP;
    CharacterAttribute->ManaMax = Data->ViewMaxMP;
    ...
    g_ConsoleDebug->Write(MCD_RECEIVE, L"0xF3:E0 [CharacterInfo nivel %d, %d puntos]",
...
void ReceiveNewCharacterCalc099B(std::span<const BYTE> ReceiveBuffer)
{
    ...
    CharacterAttribute->AttackDamageMaxRight = physicalMax;
    CharacterAttribute->AttackDamageMinLeft = physicalMin;
    CharacterAttribute->AttackDamageMaxLeft = physicalMax;

    CharacterAttribute->MagicDamageMin = static_cast<WORD>(Data->ViewMagicDamageMin);
    CharacterAttribute->MagicDamageMax = static_cast<WORD>(Data->ViewMagicDamageMax);
    CharacterAttribute->CurseDamageMin = static_cast<WORD>(Data->ViewCurseDamageMin);
    CharacterAttribute->CurseDamageMax = static_cast<WORD>(Data->ViewCurseDamageMax);

    CharacterAttribute->AttackSpeed = static_cast<WORD>(Data->ViewPhysiSpeed);
    CharacterAttribute->MagicSpeed = static_cast<WORD>(Data->ViewMagicSpeed);

    CharacterAttribute->AttackRating = static_cast<WORD>(Data->ViewAttackSuccessRate);
    CharacterAttribute->AttackRatingPK = static_cast<WORD>(Data->ViewAttackSuccessRatePvP);
    CharacterAttribute->Defense = static_cast<WORD>(Data->ViewDefense);
    CharacterAttribute->SuccessfulBlocking = static_cast<WORD>(Data->ViewDefenseSuccessRate);
    CharacterAttribute->SuccessfulBlockingPK =
        static_cast<WORD>(Data->ViewDefenseSuccessRatePvP);

    g_ConsoleDebug->Write(MCD_RECEIVE, L"0xF3:E1 [CharacterCalc dano %d-%d, defensa %d]",
        physicalMin, physicalMax, CharacterAttribute->Defense);
}

void ReceiveLife099B(std::span<const BYTE> ReceiveBuffer)
{
    auto Data = safe_cast<Mu099B::PMSG_LIFE_SEND>(ReceiveBuffer);
    if (Data == nullptr)
    {
        assert(false);
        return;
    ...
    {
    case StatCurrent:
        CharacterAttribute->Life = value;
        break;

    case StatMaximum:
        CharacterAttribute->LifeMax = value;
        break;
...
void ReceiveMana099B(std::span<const BYTE> ReceiveBuffer)
{
    auto Data = safe_cast<Mu099B::PMSG_MANA_SEND>(ReceiveBuffer);
    if (Data == nullptr)
    {
        assert(false);
        return;
    ...
    const DWORD mana = Data->ViewMP != 0 ? Data->ViewMP : ReadBigEndianWord(Data->mana);
    const DWORD bp = Data->ViewBP != 0 ? Data->ViewBP : ReadBigEndianWord(Data->bp);
...
void ReceiveGuildLeave(const BYTE* ReceiveBuffer)
{
    ...
    {
        Mu099B::SendGuildListRequest(*SocketClient);
    }
}
...
static void ProcessPacket(const BYTE* ReceiveBuffer, int32_t Size)
{
    ...
    case 0xF3:
    {
        ...
        case 0x00: //receive characters list
            ReceiveCharacterList099B(ReceiveBuffer);
            break;
        ...
        case 0x10: //receive inventory
            ReceiveInventory099B(received_span);
            break;
        ...
        case 0x07: //receive damage
            ReceiveMonsterDamage099B(received_span);
            break;
        case 0x08:
            ReceivePK(ReceiveBuffer);
            break;
        case 0x11:
            ReceiveMagicList099B(received_span);
            break;
        ...
        case 0xE1: //lo que el servidor calculo del equipo
            ...
            break;
        case 0x13:
            ReceiveEquipment099B(received_span);
            break;
        case 0x14:
            ReceiveModifyItem099B(received_span);
            break;
        case 0x20:
    ...
    case PACKET_MOVE: //move character
        ReceiveMoveCharacter099B(received_span);
        break;
    ...
    case 0x12: //create characters
        AddDebugText(ReceiveBuffer, Size);
        ReceiveCreatePlayerViewport099B(received_span);
        break;
    case 0x13: //create monsters
        //AddDebugText(ReceiveBuffer,Size);
        ReceiveCreateMonsterViewport099B(received_span);
        break;
    ...
    case 0x20: //create item
        ReceiveCreateItemViewport099B(received_span);
        break;
    case 0x2F:
    ...
    case 0x21://delete item
        ...
        break;
    case 0x22://get item
        ReceiveGetItem099B(received_span);
        break;
    case 0x23://drop item
    ...
    case 0x16://die character(exp)
        ...
        break;
    case 0x9C://experiencia y dano del golpe que mata
        ReceiveExperience099B(received_span);
        break;
    case 0x17://die character
        ReceiveDie099B(received_span, Size);
        break;
    case 0x2A:
        AddDebugText(ReceiveBuffer, Size);
        ...
        break;
    case 0x26: //life
        ReceiveLife099B(received_span);
        break;
    case 0x27: //mana y BP
        ReceiveMana099B(received_span);
        break;
    case 0x28:
    ...
    case 0x31:
        //AddDebugText(ReceiveBuffer,Size);
        ReceiveShopItemList099B(received_span);
        break;
    case 0x32:
        ReceiveBuy099B(received_span);
        break;
    case 0x33:
    ...
    case 0x1C:
        AddDebugText(ReceiveBuffer, Size);
        if (!ReceiveTeleport099B(received_span, bEncrypted))
        {
            //return ( FALSE);
        }
        break;
