...
{
    // Mismo bug que OkBtnDown -- ver ese comentario.
    Mu099B::SendPartyRequestResult(*SocketClient, false, static_cast<WORD>(PartyKey));
    PlayBuffer(SOUND_CLICK01);
    g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
{
    Mu099B::SendTradeResponse(*SocketClient, (true) != 0);
    PlayBuffer(SOUND_CLICK01);
    g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
{
    Mu099B::SendTradeResponse(*SocketClient, (false) != 0);
    PlayBuffer(SOUND_CLICK01);
    g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
{
    Mu099B::SendChaosMix(*SocketClient, static_cast<BYTE>(static_cast<ChaosMachineMixType>(g_pLuckyItemWnd->SetActAction())), 0);
    PlayBuffer(SOUND_CLICK01);
    g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
CALLBACK_RESULT SEASON3B::CMixCheckMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
    ...
    Mu099B::SendChaosMix(*SocketClient, static_cast<BYTE>(
        static_cast<ChaosMachineMixType>(g_MixRecipeMgr.GetCurMixID())), g_MixRecipeMgr.GetMixSubType());
...
CALLBACK_RESULT SEASON3B::CHighValueItemCheckMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
    ...
    {
        Mu099B::SendItemSell(*SocketClient, iSourceIndex);
...
CALLBACK_RESULT SEASON3B::CGambleBuyMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
    ...
    if (gambleSys.IsGambleShop() && BuyCost != 0)
    {
        ...
        Mu099B::SendItemBuy(*SocketClient, itemInfo.ItemIndex);
