...
bool CNewUIStorageInventory::ProcessClosing()
{
    ...
    DeleteAllItems();
    Mu099B::SendWarehouseClose(*SocketClient);
    return true;
}
...
void CNewUIStorageInventory::ProcessToReceiveStorageStatus(BYTE byStatus)
{
    ...
    case 12:
        if (IsStorageLocked() && !IsCorrectPassword())
        {
            if (m_bTakeZen)
            {
                Mu099B::SendWarehouseMoney(*SocketClient, static_cast<BYTE>(VaultMoneyMoveDirection::VaultToInventory), GetBackupTakeZen());
                InitBackupItemInfo();
            }
            else
            {
