...
bool CNewUITrade::UpdateMouseEvent()
{
        ...
        || (m_pMyInvenCtrl && false == m_pMyInvenCtrl->UpdateMouseEvent()))
    {
        ...
        {
            m_bMyConfirm = false;
            Mu099B::SendTradeOkButton(*SocketClient, (TradeButtonState::Unchecked) == TradeButtonState::Checked);
        }
...
bool CNewUITrade::UpdateKeyEvent()
{
    if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_TRADE) == true)
    {
        ...
        {
            Mu099B::SendTradeCancel(*SocketClient);
...
void CNewUITrade::SendRequestMyGoldInput(int nInputGold)
{
    if (nInputGold <= (int)CharacterMachine->Gold + m_nMyTradeGold)
    {
        ...
        Mu099B::SendTradeMoney(*SocketClient, nInputGold);
    }
    else
    {
...
void CNewUITrade::AlertTrade()
{
    ...
    m_bTradeAlert = true;
    Mu099B::SendTradeOkButton(*SocketClient, (m_bMyConfirm ? TradeButtonState::Checked : TradeButtonState::Unchecked) == TradeButtonState::Checked);
}
