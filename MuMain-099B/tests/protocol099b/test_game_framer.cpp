// Separador de paquetes del socket de GameServer: acá se juntan las tres capas
// (cifrado de flujo, cifrado por bloques y XorData) en el orden correcto.
//
// Los casos arman el flujo tal como lo emitiría el servidor y comprueban que el
// framer recupere el paquete lógico exacto. El punto que más importa es que el
// flujo pueda partirse en cualquier lado: el socket entrega lo que quiere, no
// paquetes completos.
...
// Data/Hack/Enc2.dat -- con esto cifra el SERVIDOR.
...
/// Arma un C1 plano tal como sale del servidor: SIN XorData (el envío nunca la
/// aplica) y pasado por el cifrado de flujo.
...
{
    // El socket entrega lo que quiere: mitad de un encabezado, tres paquetes
    // juntos, un byte suelto. El framer tiene que aguantar todo eso.
    auto framer = MakeFramer();
...
{
    // Si el tipo no es C1/C2/C3/C4 el flujo ya está desincronizado: seguir
    // adivinando sólo entrega basura como si fuera protocolo.
    auto framer = MakeFramer();
...
{
    // Camino completo de un C3 tal como lo emite el servidor: se cifra con
    // Enc2.dat (rol servidor) y el cliente lo descifra con Dec2.dat. El primer
    // byte del bloque es el número de serie, no el head.
    ...
    // Se reconstruye como C1 lógico, con el cuerpo original ya des-ofuscado.
    CHECK(p.Data[0] == 0xC1);
...
{
    // El checksum es lo único que distingue un paquete legítimo de ruido; con
    // tablas que no hacen juego tiene que fallar, no entregar basura.
